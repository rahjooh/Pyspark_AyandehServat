...
def Servat_insert(date1):
    ...
    # read second Table
    custinfo97_01_DF = spark.read.parquet("hdfs://10.100.136.60:9000/user/hduser/pqCUSTINFO")
    ...
    mvCus1_DF = spark.sql("""
                 select sum(substr(b.REMAININGAMOUNTEFFECTIVE, 0, length(b.REMAININGAMOUNTEFFECTIVE)-1)) as jam,
                        int(((substring(current_date(),0,4) *365) +(substring(current_date(),5,2) *30.42) + substring(current_date(),7,2))- 226746.26 - ((substring(concat('13',c.DATEOPN),1,4) *365 ) + (substring(c.DATEOPN,3,2)*30.42 ) +  substring(c.DATEOPN,5,2))) as ghedmat,
                        b.custno
                from lastbal9697 b  
    ...
    print(1)
    # mvCus4_DF.write.save(path='csv', format='csv', mode='append', sep=',')
    with open('servat_daily.csv', 'a') as f1:
    # mvCus4_DF.write.csv('mycsv.csv')
        pdsDF.to_csv(f1, header=False ,encoding='utf-8')
    print(2)
